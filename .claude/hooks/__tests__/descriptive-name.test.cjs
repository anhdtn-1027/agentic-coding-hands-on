...
describe('descriptive-name.cjs', () => {
  ...
  describe('Hook disable functionality', () => {
    ...
    it('exits silently when disabled via .tkm.json', async () => {
      // Create .claude/.tkm.json with hook disabled
      ...
      fs.copyFileSync(
        path.join(__dirname, '..', 'lib', 'tkm-config-utils.cjs'),
        path.join(libDir, 'tkm-config-utils.cjs')
      );
