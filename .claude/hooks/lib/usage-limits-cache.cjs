...
function getUsageCachePath() {
  return process.env.TKM_USAGE_CACHE_PATH || path.join(os.tmpdir(), 'sk-usage-limits-cache.json');
}

function getQuotaEligibilityCachePath() {
  return process.env.TKM_USAGE_ELIGIBILITY_CACHE_PATH || `${getUsageCachePath()}.eligibility`;
}
