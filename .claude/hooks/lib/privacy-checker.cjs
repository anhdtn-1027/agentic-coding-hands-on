...
/**
 * Load .tkm.json config (deprecated .sk.json fallback) to check if privacy block is disabled
...
function isPrivacyBlockDisabled(configDir) {
  ...
  // .tkm.json preferred; .sk.json kept as deprecated read-only fallback
  for (const name of ['.tkm.json', '.sk.json']) {
    try {
      ...
      // Try next candidate; default to enabled when none readable
    }
  }
  return false;
}
