...
// Deprecated .sk.json paths kept as read-only fallback so existing user configs keep working
...
/**
 * Load config from a deprecated .sk.json path.
 * Intentionally silent: hooks (statusline included) run on every prompt and must
 * not write to stderr — the .sk.json deprecation is announced in release notes instead.
 * @param {string} legacyPath - Path to deprecated config file
 * @returns {Object|null} Parsed config or null if not found/invalid
 */
function loadLegacyConfig(legacyPath) {
 ...
 *   2. Global config (~/.claude/.tkm.json) - user preferences (.sk.json deprecated fallback)
 *   3. Local config (./.claude/.tkm.json) - project-specific overrides (.sk.json deprecated fallback)
 *
...
function loadConfig(options = {}) {
  ...
  // Load configs from both locations; fall back to deprecated .sk.json when .tkm.json is absent
  const globalConfig = loadConfigFromPath(GLOBAL_CONFIG_PATH) || loadLegacyConfig(LEGACY_GLOBAL_CONFIG_PATH);
  const localConfig = loadConfigFromPath(LOCAL_CONFIG_PATH) || loadLegacyConfig(LEGACY_LOCAL_CONFIG_PATH);
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.TKM_DEBUG) {
      console.error(`[tkm-config] Warning: ${validation.error}`);
    }
  }
