/**
 * parity.test.mjs — cross-language parity guard for the 4 deterministic steps.
 *
 * The lib/*.mjs CLIs are 1:1 behavioural ports of the upsale skill's Python step
 * scripts (claude/skills/upsale/scripts/*.py). The other __tests__ exercise the JS
 * cores in isolation; THIS suite proves the JS CLIs still reproduce the Python CLIs
 * BYTE-FOR-BYTE, so future edits to either side can't silently drift apart.
 *
 ...
 *
 * The Python CLIs (detect_sdd.py / combine_proposals.py / phase_d_prep.py /
 * apply_verdicts.py) and their *_lib.py modules import stdlib only, so any `python3`
 * on PATH works — no venv required. If no python3 (or the upsale scripts) are present
 * the whole suite SKIPS rather than fails, keeping CI green on minimal images.
 */
...
const PY_SCRIPTS = path.join(REPO_ROOT, 'claude/skills/upsale/scripts');
...
const SKIP = TOOLCHAIN_OK ? false : 'python3 or upsale scripts unavailable — parity check skipped';
 ...
 *  - mask the ONE intentional rebrand string: the combined-proposal H1 was renamed
 *    "Upsale Proposal" → "Improvement Proposal" in the port. That rename is deliberate
 *    and user-facing; everything else (structure, transforms, sort, dedup, evidence,
 *    path handling) must still match the Python source byte-for-byte.
 */
...
// steps, propose-improvements' phase-d-prep.mjs has diverged from the upsale skill's
...
// self-verifies against the repo. The Python engine stays fat for the upsale skill.
