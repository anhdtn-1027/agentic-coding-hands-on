 ...
 * Rendering is config-driven via statuslineLayout in .tkm.json.
...
// Read stdin with optional inactivity timeout (TKM_STATUSLINE_STDIN_TIMEOUT_MS)
async function readStdin() {
  return new Promise((resolve, reject) => {
    const chunks = [];
    stdin.setEncoding('utf8');
    const parsedTimeout = Number.parseInt(env.TKM_STATUSLINE_STDIN_TIMEOUT_MS || '', 10);
