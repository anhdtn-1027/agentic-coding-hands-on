...
Agent(
  ...
  prompt: "full instructions + TKM Context Block",
...
```
TKM Context:
- Work dir: {TKM_PROJECT_ROOT or CWD}
- Reports: {TKM_REPORTS_PATH or "plans/reports/"}
- Plans: {TKM_PLANS_PATH or "plans/"}
- Branch: {TKM_GIT_BRANCH or current branch}
- Naming: {TKM_NAME_PATTERN or "YYMMDD-HHMM"}
- Active plan: {TKM_ACTIVE_PLAN or "none"}
...
3. **CALL** `TaskCreate` x N -- one per angle:
   ...
   - Description: `Investigate <angle> for topic: <topic>. Save report to: {TKM_REPORTS_PATH}/researcher-{N}-{TKM_NAME_PATTERN}-{topic-slug}.md. Format: Executive summary, key findings, evidence, recommendations. Mark task completed when done. Send findings summary to lead.`
...
4. **SPAWN** teammates x N via `Agent` tool:
   ...
   - `name: "researcher-{N}"`
   - Prompt: task description + TKM Context Block
...
6. **READ** all researcher reports from `{TKM_REPORTS_PATH}/`

7. **SYNTHESIZE** into: `{TKM_REPORTS_PATH}/research-summary-{TKM_NAME_PATTERN}-{topic-slug}.md`
...
4. **SPAWN** developer teammates x N via `Agent` tool:
   ...
   - `name: "dev-{N}"`
   - Prompt: task description + plan context + TKM Context Block
...
3. **CALL** `TaskCreate` x N -- one per focus:
   ...
   - Description: `Review <scope> for <focus>. Output severity-rated findings only. Format: [CRITICAL|IMPORTANT|MODERATE] <finding> -- <evidence> -- <recommendation>. No "seems" or "probably" -- concrete evidence only. Save to: {TKM_REPORTS_PATH}/reviewer-{N}-{TKM_NAME_PATTERN}-{scope-slug}.md. Mark task completed when done.`
...
6. **SYNTHESIZE** into: `{TKM_REPORTS_PATH}/review-{scope-slug}.md`
...
7. **WRITE** root cause report: `{TKM_REPORTS_PATH}/debug-{issue-slug}.md`
