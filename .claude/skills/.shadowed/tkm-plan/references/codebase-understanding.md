...
2. **Step B — Rebuild-spec shape:** if `docs/generated/feature-list.md` exists, that's the canonical entry point for feature work (v4.0.0 layered layout).
   - Read `docs/generated/feature-list.md` first → find the feature relevant to the task.
   - Drill into `docs/features/{slug}/technical-spec.md` for that feature only.
   - Top-level cross-cutting context lives in `docs/system/overview.md`, `docs/generated/entities.md`, `docs/generated/screen-flow.md`, etc. — read only what the task touches.
3. **Step C — Flat-topic shape:** if `docs/*.md` files exist at the top level (no layered `system/`/`generated/` subdirs), browse by topic. Read whichever file matches the task domain.
