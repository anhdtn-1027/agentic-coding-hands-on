...
- Gate: skip when `--business-only` (technical inactive) or under `--debug`.
...
- **Step 4.1.06 (`06-security-compliance`) additionally receives `nyx_ready: <bool>`** from Preflight; all other discovery items ignore it. (4.1.06 is an always-present tech item, not `--high`-gated.)
