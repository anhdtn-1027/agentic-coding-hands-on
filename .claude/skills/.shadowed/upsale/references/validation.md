...
As part of check 1 (and persisted regardless of decision), decompose BOTH the item's `**Need:**` bullet AND its `**Proposed solution:**` bullet into atomic claims and record ONE audit entry per claim. A "claim" is an independently verifiable assertion — split each bullet on `;`, `.`, and conjunctions whenever each fragment carries an independently checkable assertion. Audits cover those two bullets only — Value/Use-context/Benefits/Security/Formatting verdicts are summarised in `# Reason` by citing the check number, not added to `# Audits`.

Each audit entry captures:

- **Clause** — the atomic claim verbatim/lightly-paraphrased from the source bullet (preserve every `path:line`, version, advisory ID, route, and metric exactly). MUST start with `(need)` or `(solution)` — the tag identifies which bullet the claim came from.
- **Evidence** — what was inspected to verify the claim, with the supporting **repo** `path:line` (see Evidence source rule below), or the search performed and its negative result when unsupportable.
- **Verdict** — `correct` when the cited evidence supports the claim, or `wrong` when the citation is stale/fabricated, the version/ID does not match the lockfile, the metric is absent, the proposed library/API/path/version is unsupportable on the detected stack, or the assertion contradicts the repo.

Each claim's verification direction:

- `(need)` → backward-looking repo state (what already exists). A `(need)` claim proposing to re-enable / restore commented-out or disabled code is **wrong by default** — in a shipped product the disable was deliberate, so REVISE toward removing the dead code (and dropping any dependency only it keeps alive). Only exception: the in-repo spec still marks the feature active, or an open TODO/issue flags it as temporarily disabled.
- `(solution)` → forward-looking stack-fit: a library/dep against lockfile or manifest; a file path against repo tree and folder conventions; an API symbol against the cited library version; a version pin against existing peer ranges; a pattern/refactor against repo grep for prior implementation; a config/env key against the env schema. A purely high-level solution (no specific library / path / API / version named) emits exactly ONE `(solution)` claim stating the high-level pattern, with evidence that the stack accepts (or rejects) the pattern — don't manufacture sub-claims.

Always emit `# Audits`, including on DROP, on items with no `**Need:**` bullet (emit a single `(need) Need bullet absent or empty` clause with `wrong` verdict), and on items with no `**Proposed solution:**` bullet (emit a single `(solution) Proposed solution bullet absent or empty` clause with `wrong` verdict).
...
Read the title plus every bullet (`Value`, `Need`, `Benefits`, `Proposed solution`, `Engineering effort hint`) together. Use `item_evidence` to learn *which* claims to check and *which repo paths* they point at, then verify each claim against the **actual repo** (see "Evidence source rule" above). Form one end-to-end judgment by asking:
...
If REVISE: keep exact schema — `## <title>` heading followed by `**Value:**`, `**Need:**`, `**Benefits:**`, `**Proposed solution:**`, `**Engineering effort hint:**` in that order. No extra bullets (`**Category:**` is no longer part of the schema — omit it), no heading changes. **Bullets-only body** — emit NO sub-headings (`###`, `####`, `#####`, `######`) inside the body; bodies with any sub-heading are rejected by apply (`revise-malformed` → KEEP fallback). If the original item used sub-headings to organise prose, fold the prose into the relevant `**Label:**` bullet instead.
...
<Omit when KEEP or DROP. When REVISE, emit full revised item starting with `## <title>`, five bullets in order: Value, Need, Benefits, Proposed solution, Engineering effort hint.>
