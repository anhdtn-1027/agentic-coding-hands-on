...
def estimate(
    ...
    if artifact in ("api-contracts", "api-map"):
        ...
        if route_list:  # incremental rerun: route-list.md already exists
            unit_count = _count_route_data_rows(route_list)
        if unit_count == 0 and scout_report:  # first-gen: only the scout report exists
            unit_count = _count_inventory_by_type(scout_report, "route") * ROUTE_ROWS_PER_FILE
