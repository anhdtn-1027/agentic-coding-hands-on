...
3. Pad each actor's range by +20% headroom → ranges still disjoint. Unused tail gaps SAFE (no contiguity/density gate; W4.5 is uniqueness-only).
...
- **Merge → F### integrity gate, THEN existing W5 post-step:** Before the canonical-fcode derivation, assert every `### F###` in the merged file appears in `_slice-plan.json`. A fan-out chunk that hallucinates an out-of-batch F### would otherwise flow straight into a spurious `.pending` feature folder. After the gate: parse `### F###` → slug derivation → GLOBAL slug-collision check → sorted `_canonical-fcodes.json` → `.pending` folders (UNCHANGED — post-step already runs "after feature-list.md finalized").
