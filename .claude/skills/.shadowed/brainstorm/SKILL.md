...
license: MIT
argument-hint: "[topic or problem] [--role ceo|cto|cfo|coo|cmo|cpo] [--bod[=r1,r2,...]] [--level low|medium|high|max]"
metadata:
...
1. **Read the Workshop** — Use `tkm:scan-codebase` to understand the current codebase and read relevant docs in `<project-dir>/docs`. Know the material before examining the commission. *(For non-technical commissions or non-CTO lenses, scout whatever material exists — docs, briefs, prior plans — instead of code.)*
