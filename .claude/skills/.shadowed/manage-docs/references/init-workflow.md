...
`init` does NOT scaffold the spec layer. The machine-generated spec layer (`docs/system/`, `docs/generated/`, `docs/features/`, `docs/flows/`) is produced by `/tkm:rebuild-spec` after the codebase has measurable scope (routes, models, screens). If a spec layer (or a legacy `docs/specs/` tree) already exists at init time, leave it untouched. See `claude/skills/_shared/docs-canonical-mapping.md` for the layered model.
