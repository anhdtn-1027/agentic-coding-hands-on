...
**Absent-layer advisory:** the `if/elif` block above is stderr-only and **never** enters the `doc-writer` prompt — surgical-edit contract is unchanged. Layers are mutually exclusive: `docs/` missing suppresses the spec-layer advisory because the layered namespaces (`docs/system/`, `docs/generated/`, …) cannot exist without their `docs/` parent. Threshold `≥ 2` is applied **after** stripping test/mock/fixture paths. Contract: [`_shared/docs-canonical-mapping.md` § Absent-Layer Advisory](../../_shared/docs-canonical-mapping.md#absent-layer-advisory).
