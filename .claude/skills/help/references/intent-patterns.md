...
> **Depth signal (`--level`):** if the user asks for a "quick/light" pass or a "thorough/exhaustive/critical" pass on a depth-sensitive skill (`research`, `scan-codebase`, `brainstorm`, `review-code`, `audit-security`, `debug-code`, `predict-risks`, `upsale`), append `--level low|medium|high|max` (default `medium`).
...
### Render a Mermaid diagram → `/tkm:draw-diagram`
**English triggers:**
