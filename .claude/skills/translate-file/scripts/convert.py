...
def _convert_docx_native(input_file, temp_dir, chunk_size, ilang, olang):
    """Extract DOCX natively with python-docx (Idea A). Returns True on success."""
    try:
...
def main():
    ...
    if file_ext not in ['.pdf', '.docx', '.epub', '.xlsx']:
    ...
    if args.mode != 'markdown':
        ...
        if file_ext == '.xlsx':
            ...
            print("Trying PDF native pipeline (pdf2docx → python-docx)...")
