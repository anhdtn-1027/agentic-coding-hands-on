...
def _build_xlsx_native(temp_dir):
    ...
    build_script = os.path.join(SCRIPT_DIR, "build_xlsx.py")
...
def _build_docx_native(temp_dir):
    """Idea A/B: rebuild DOCX directly from docx_structure.json + output.md.

    Calls build_docx.py via subprocess. Returns True on success.
    """
    ...
    build_script = os.path.join(SCRIPT_DIR, "build_docx.py")
    if not os.path.exists(build_script):
    ...
    try:
        cmd = ["python3", build_script, source_docx, "--temp-dir", temp_dir, "--output", output_docx]
        result = subprocess.run(cmd, check=True, capture_output=True, text=True)
        if result.stdout:
            print(result.stdout.strip())
