...
interface LanguageDropdownProps {
  onClose: () => void;
  // Ref to the switcher wrapper (trigger button + this dropdown). Outside-click
  // is checked against the whole wrapper so clicking the trigger to dismiss is
  // handled by the button's own toggle — not closed-then-reopened here.
  containerRef: RefObject<HTMLDivElement | null>;
}

export function LanguageDropdown({ onClose, containerRef }: LanguageDropdownProps) {
      ...
      {LANGUAGE_OPTIONS.map((option) => {
        ...
        return (
          <button
            ...
            // Selected: bg rgba(255,234,158,0.20) on a 2px-rounded highlight (mm VN node).
            // Unselected: transparent, 0px radius (mm EN node). Content kept left-aligned
            // for both (design's per-option justify maps to state — avoids shift on switch).
            className="flex flex-row items-center justify-start w-full cursor-pointer transition-colors"
            style={{
              padding: "16px",
